...
def parse_args():
    ...
    parser_mail.add_argument("--mailfrom", default=None, type=str, help="sender address")
    ...
    parser.add_argument("--silent", "-s", action="store_true", help="don't print report to stdout")
        ...
        for item in reports:
            ...
            try:
                ...
                if item["resource"].get("answers"):
                    result["answers"] = list(chain.from_iterable([i["answer"] for i in item["resource"]["answers"]]))
                else:
...
def send_mail(mailfrom, mailto, server, user, password, body, formatting=None, port=465):
    ...
    if formatting == "html":
        ...
        env = Environment(
            loader=FileSystemLoader(Path(__file__).resolve().parent.joinpath("templates")),
            autoescape=select_autoescape(['html'])
        )
        template = env.get_template('nsone-report.html.j2')
        html_part = MIMEText(template.render(body=body, title="NS1 Activity Report"), "html", "utf-8")
...
if __name__ == "__main__":
    ...
    try:
        logging.basicConfig(format='%(time)s | %(levelname)-8s | %(message)s',
                            filename=log_path,
                            filemode='a',
                            level=logging.DEBUG)
